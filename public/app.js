...
function crearFormularioLista(e){
    ...
    agregarbtn.addEventListener('click', async _ =>{
        ...
        const response = await request('http://localhost:8000/lista','POST',{titulo: nuevoTitulo})
...
function crearFormulario(e){
    ...
    agregarbtn.addEventListener('click', async _ =>{
        ...
        const response = await request('http://localhost:8000/lista/carta','POST',{titulo,descripcion, id: lista.dataset.id})
...
async function finalizarEdicionCarta(e){
    ...
    const contenidoCarta = [...cartaEnEdicion.children]
    await request('http://localhost:8000/carta','PATCH',{titulo, descripcion, id: carta.dataset.id})
...
async function finalizarEdicionLista(e){
    ...
    await request('http://localhost:8000/lista','PATCH',{id: wrapper.parentNode.dataset.id, titulo: input.value})
...
async function eliminarCarta(e){
    ...
    if(confirm('¿Seguro qué deseas eliminar la carta?\nNo la vas a poder recuperar')){
        await request('http://localhost:8000/carta','DELETE',{cartaId: carta.dataset.id, listaId: carta.parentNode.parentNode.dataset.id})
...
async function eliminarLista(e){
    ...
    if(confirm('¿Seguro que desea eliminar la lista?\nTodas las cartas adentro de la lista tambien serán eliminadas para siempre')){
        await request('http://localhost:8000/lista', 'DELETE',{id : lista.dataset.id})
...
async function dragEnd(e){
    ...
    if(listaFinal == listaInicial){
        ...
        await request('http://localhost:8000/lista','PATCH', {id: listaInicial.dataset.id, cartas: cartas.map(carta => carta.dataset.id)})
    }else{
        ...
        await request('http://localhost:8000/lista/s','PATCH',{
            lista1:{
...
const iniciar = async () =>{
    const respuesta = await request('http://localhost:8000/lista','get')
